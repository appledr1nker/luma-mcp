...
// Alpic injects user-supplied request headers into extra.requestInfo.headers
// regardless of transport type, so BYO key works the same as with HTTP.
...
async function fetchAllPeople(apiKey, { calendar_api_id, cutoff } = {}) {
  ...
  while (true) {
    const res = await luma(apiKey, "/v1/calendar/list-people", {
      calendar_api_id, sort_column: "created_at", sort_direction: "asc",
...
async function fetchAllEvents(apiKey, { calendar_api_id, after, before } = {}) {
  ...
  while (true) {
    const res = await luma(apiKey, "/v1/calendar/list-events", {
      calendar_api_id, after, before, pagination_limit: 50, pagination_cursor: cursor,
    });
    if (!res.entries?.length) break;
...
async function fetchAllGuests(apiKey, event_id) {
  ...
  while (true) {
    const res = await luma(apiKey, "/v1/event/get-guests", {
      event_api_id: event_id, pagination_limit: 50, pagination_cursor: cursor,
    });
    if (!res.entries?.length) break;
...
server.tool("list_events", "List calendar events (trimmed)", {
  ...
  return { content: [{ type: "text", text: JSON.stringify({ events: res.entries?.map(trimEvent) ?? [], has_more: res.has_more, next_cursor: res.next_cursor }, null, 2) }] };
});
...
server.tool("list_people", "List people in a calendar (trimmed)", {
  ...
  return { content: [{ type: "text", text: JSON.stringify({ people: res.entries?.map(trimPerson) ?? [], has_more: res.has_more, next_cursor: res.next_cursor }, null, 2) }] };
});
...
server.tool("get_guests", "List guests for an event (trimmed)", {
  ...
  return { content: [{ type: "text", text: JSON.stringify({ guests: res.entries?.map(trimGuest) ?? [], has_more: res.has_more, next_cursor: res.next_cursor }, null, 2) }] };
});
...
server.tool("get_revenue_over_time", "Cumulative revenue from subscribers over time, bucketed by subscriber join date.", {
  ...
  for (const [date, { new_revenue_cents, new_subscribers }] of Object.entries(byBucket).sort()) {
    cumRevenue += new_revenue_cents; cumSubs += new_subscribers;
    series.push({ date, new_revenue_usd: (new_revenue_cents / 100).toFixed(2), total_revenue_usd: (cumRevenue / 100).toFixed(2), new_subscribers, total_subscribers: cumSubs });
  }
  const totalRevenueCents = people.reduce((s, p) => s + (p.revenue_usd_cents ?? 0), 0);
  return { content: [{ type: "text", text: JSON.stringify({ total_subscribers: people.length, total_revenue_usd: (totalRevenueCents / 100).toFixed(2), bucket, series }, null, 2) }] };
});
...
server.tool("get_attendance_over_time", "Registrations and check-ins per time bucket across events.", {
  ...
  const k = getKey(extra);
  const events = (await fetchAllEvents(k, { calendar_api_id, after, before })).slice(0, max_events);
  const results = await Promise.all(events.map(async (raw) => {
    const ev = raw.event ?? raw;
    const guests = await fetchAllGuests(k, ev.id);
    return { event: trimEvent(ev), registered: guests.filter(g => g.approval_status === "approved").length, checked_in: guests.filter(g => g.event_tickets?.some(t => t.check_in_at)).length };
  }));
  const byBucket = {};
  for (const { event, registered, checked_in } of results) {
    ...
    byBucket[bk].events += 1; byBucket[bk].registrations += registered; byBucket[bk].check_ins += checked_in;
  }
  const series = Object.entries(byBucket).sort().map(([date, s]) => ({ date, ...s, avg_registrations_per_event: s.events > 0 ? Math.round(s.registrations / s.events) : 0, avg_show_rate_pct: s.registrations > 0 ? Number((s.check_ins / s.registrations * 100).toFixed(1)) : 0 }));
...
server.tool("get_event_summary", "Full performance summary for one event: attendance, show rate, UTM source breakdown.", {
  ...
  const k = getKey(extra);
  const [eventRes, guests] = await Promise.all([luma(k, "/v1/event/get", { api_id: event_api_id }), fetchAllGuests(k, event_api_id)]);
  ...
  const utmBreakdown = {}, ticketBreakdown = {};
  for (const g of approved) {
    const src = g.utm_source ?? "(direct)";
    utmBreakdown[src] = (utmBreakdown[src] ?? 0) + 1;
    for (const t of g.event_tickets ?? []) { const key = t.type_id ?? "default"; ticketBreakdown[key] = (ticketBreakdown[key] ?? 0) + 1; }
  }
  return { content: [{ type: "text", text: JSON.stringify({ event: trimEvent(eventRes), stats: { total_guests: guests.length, registered: approved.length, checked_in: checkedIn.length, show_rate_pct: approved.length > 0 ? Number((checkedIn.length / approved.length * 100).toFixed(1)) : 0, utm_sources: utmBreakdown, ticket_types: ticketBreakdown } }, null, 2) }] };
});
...
server.tool("list_events_with_stats", "Recent events enriched with registration and check-in counts.", {
  ...
  const k = getKey(extra);
  const events = (await fetchAllEvents(k, { calendar_api_id, after, before })).slice(0, limit);
  const enriched = await Promise.all(events.map(async (raw) => {
    const ev = raw.event ?? raw;
    const guests = await fetchAllGuests(k, ev.id);
...
server.tool("get_subscriber_breakdown", "Subscribers grouped by tag — understand your audience segments.", {
  ...
  const k = getKey(extra);
  const people = await fetchAllPeople(k, { calendar_api_id });
  const byTag = {}; let untagged = 0;
  for (const p of people) {
    if (!p.tags?.length) { untagged++; continue; }
    for (const tag of p.tags) byTag[tag.name] = (byTag[tag.name] ?? 0) + 1;
  }
  const breakdown = Object.entries(byTag).sort(([, a], [, b]) => b - a).map(([tag, count]) => ({ tag, count, pct: Number((count / people.length * 100).toFixed(1)) }));
...
server.tool("get_repeat_attendees", "Subscribers who attended more than one event — your most engaged audience.", {
  ...
  return { content: [{ type: "text", text: JSON.stringify({ total_repeat_attendees: qualified.length, pct_of_subscribers: Number((qualified.length / people.length * 100).toFixed(1)), min_events, attendees: top }, null, 2) }] };
});
...
server.tool("marketing_dashboard", "High-level marketing overview: subscriber totals, MoM growth, event counts, revenue. Single call.", {
  ...
  const k = getKey(extra);
  const [people, events] = await Promise.all([fetchAllPeople(k, { calendar_api_id }), fetchAllEvents(k, { calendar_api_id })]);
  const now = new Date();
  ...
  return {
    content: [{
      type: "text",
      text: JSON.stringify({
        subscribers: { total: people.length, new_this_month: thisMonthSubs, new_last_month: lastMonthSubs, mom_growth_pct: lastMonthSubs > 0 ? Number(((thisMonthSubs - lastMonthSubs) / lastMonthSubs * 100).toFixed(1)) : null, repeat_attendees: repeatCount, repeat_attendee_pct: Number((repeatCount / people.length * 100).toFixed(1)) },

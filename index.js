...
// Field trimmers — strips verbose API responses down to marketing-relevant fields
...
function trimEvent(raw) {
  // list-events wraps events in { event: {...} }; get returns the object directly
...
async function fetchAllPeople({ calendar_api_id, cutoff } = {}) {
  const people = [];
  let cursor = undefined;
  while (true) {
    const res = await luma("/v1/calendar/list-people", {
      calendar_api_id,
...
async function fetchAllEvents({ calendar_api_id, after, before } = {}) {
  const events = [];
  let cursor = undefined;
  while (true) {
    const res = await luma("/v1/calendar/list-events", {
      calendar_api_id,
      after,
      before,
      pagination_limit: 50,
...
async function fetchAllGuests(event_id) {
  const guests = [];
  let cursor = undefined;
  while (true) {
    const res = await luma("/v1/event/get-guests", {
...
// ─── Core tools (trimmed responses) ──────────────────────────────────────────

server.tool("get_self", "Get authenticated user info", {}, async () => ({
  content: [{ type: "text", text: JSON.stringify(await luma("/v1/user/get-self"), null, 2) }]
}));

server.tool("get_calendar", "Get calendar details", {
  api_id: z.string().optional(),
}, async (p) => ({
  content: [{ type: "text", text: JSON.stringify(await luma("/v1/calendar/get", p), null, 2) }]
}));
...
server.tool("list_events", "List calendar events (trimmed)", {
  ...
  const res = await luma("/v1/calendar/list-events", p);
  return {
    content: [{
      type: "text",
      text: JSON.stringify({
...
server.tool("get_event", "Get details about a specific event", {
  ...
  const res = await luma("/v1/event/get", { api_id });
  return { content: [{ type: "text", text: JSON.stringify(trimEvent(res), null, 2) }] };
});
...
server.tool("list_people", "List people in a calendar (trimmed)", {
  ...
  const res = await luma("/v1/calendar/list-people", p);
  return {
    content: [{
      type: "text",
      text: JSON.stringify({
...
server.tool("get_guests", "List guests for an event (trimmed)", {
  ...
  const res = await luma("/v1/event/get-guests", p);
  return {
    content: [{
      type: "text",
      text: JSON.stringify({
...
server.tool("get_guest", "Get a specific guest", {
  ...
  const res = await luma("/v1/event/get-guest", p);
  return { content: [{ type: "text", text: JSON.stringify(trimGuest(res), null, 2) }] };
});

server.tool("list_ticket_types", "List ticket types for an event", {
  event_api_id: z.string(),
}, async (p) => ({
  content: [{ type: "text", text: JSON.stringify(await luma("/v1/event/ticket-types/list", p), null, 2) }]
}));
...
server.tool("list_coupons", "List coupons for an event or calendar", {
  ...
  return { content: [{ type: "text", text: JSON.stringify(await luma(path, params), null, 2) }] };
});
...
server.tool("list_org_events", "List events across all org calendars", {
  ...
  content: [{ type: "text", text: JSON.stringify(await luma("/v1/organizations/events/list", p), null, 2) }]
}));
...
server.tool("get_subscribers_over_time", "Cumulative subscriber growth as a time series. Paginates internally.", {
  ...
  const people = await fetchAllPeople({ calendar_api_id, cutoff: created_before });
  const series = buildCumulativeSeries(people.map(p => p.created_at), bucket);
  return {
    content: [{
      type: "text",
      text: JSON.stringify({ total: people.length, bucket, series }, null, 2)
    }]
  };
});
...
server.tool("get_events_over_time", "Event volume trend as a cumulative time series.", {
  ...
  const events = await fetchAllEvents({ calendar_api_id, after, before });
  const dates = events.map(e => (e.event ?? e)[date_field]).filter(Boolean);
  const series = buildCumulativeSeries(dates, bucket);
  return {
    content: [{
      type: "text",
      text: JSON.stringify({ total: events.length, bucket, date_field, series }, null, 2)
    }]
  };
});
...
server.tool("get_revenue_over_time", "Cumulative revenue from subscribers over time, bucketed by subscriber join date.", {
  ...
  const people = await fetchAllPeople({ calendar_api_id });

  const byBucket = {};
  for (const p of people) {
    const k = bucketKey(new Date(p.created_at), bucket);
    if (!byBucket[k]) byBucket[k] = { new_revenue_cents: 0, new_subscribers: 0 };
    byBucket[k].new_revenue_cents += p.revenue_usd_cents ?? 0;
    byBucket[k].new_subscribers += 1;
  }
...
server.tool("get_attendance_over_time", "Registrations and check-ins per time bucket across events. Fetches guest counts per event — may be slow for large date ranges.", {
  calendar_api_id: z.string().optional(),
  after: z.string().optional().describe("ISO 8601"),
  before: z.string().optional().describe("ISO 8601"),
  ...
  const allEvents = await fetchAllEvents({ calendar_api_id, after, before });
  const events = allEvents.slice(0, max_events);

  const results = await Promise.all(
    events.map(async (raw) => {
      const ev = raw.event ?? raw;
      const guests = await fetchAllGuests(ev.id);
  ...
  for (const { event, registered, checked_in } of results) {
    const k = bucketKey(new Date(event.start_at), bucket);
    if (!byBucket[k]) byBucket[k] = { events: 0, registrations: 0, check_ins: 0 };
    byBucket[k].events += 1;
    byBucket[k].registrations += registered;
    byBucket[k].check_ins += checked_in;
  }
  ...
  return {
    content: [{
      type: "text",
      text: JSON.stringify({ events_analyzed: results.length, bucket, series }, null, 2)
    }]
  };
});
...
server.tool("get_event_summary", "Full performance summary for one event: attendance, show rate, UTM source breakdown.", {
  ...
  const [eventRes, guests] = await Promise.all([
    luma("/v1/event/get", { api_id: event_api_id }),
    fetchAllGuests(event_api_id),
  ]);
  ...
  const showRate = approved.length > 0
    ? Number((checkedIn.length / approved.length * 100).toFixed(1))
    : 0;
...
server.tool("list_events_with_stats", "Recent events enriched with registration and check-in counts.", {
  ...
  const enriched = await Promise.all(
    events.map(async (raw) => {
      ...
      return {
        ...
        show_rate_pct: registered > 0 ? Number((checkedIn / registered * 100).toFixed(1)) : 0,
      };
    })
  );
  ...
  return {
    content: [{
      type: "text",
      text: JSON.stringify({ total: enriched.length, events: enriched }, null, 2)
    }]
  };
});
...
server.tool("get_subscriber_breakdown", "Subscribers grouped by tag — understand your audience segments.", {
  ...
  return {
    content: [{
      type: "text",
      text: JSON.stringify({ total_subscribers: people.length, untagged, breakdown }, null, 2)
    }]
  };
});
...
server.tool("get_repeat_attendees", "Subscribers who attended more than one event — your most engaged audience.", {
  ...
  const top = qualified
    .sort((a, b) => (b.event_checked_in_count ?? 0) - (a.event_checked_in_count ?? 0))
    .slice(0, limit)
    .map(trimPerson);
...
server.tool("marketing_dashboard", "High-level marketing overview: subscriber totals, MoM growth, event counts, revenue. Single call.", {
  ...
  const [people, events] = await Promise.all([
    fetchAllPeople({ calendar_api_id }),
    fetchAllEvents({ calendar_api_id }),
  ]);
  ...
  const lastMonthSubs = people.filter(p => {
    const d = new Date(p.created_at);
    return d >= lastMonthStart && d < thisMonthStart;
  }).length;
  const momGrowthPct = lastMonthSubs > 0
  ...
  return {
    content: [{
      ...
      text: JSON.stringify({
        subscribers: {
          total: people.length,
          new_this_month: thisMonthSubs,
          new_last_month: lastMonthSubs,
          mom_growth_pct: momGrowthPct,
          repeat_attendees: repeatCount,
          repeat_attendee_pct: Number((repeatCount / people.length * 100).toFixed(1)),
        },
        events: {
          total: events.length,
          upcoming,
        ...
        revenue: {
          ...
          avg_per_subscriber_usd: people.length > 0
            ? (totalRevenueCents / people.length / 100).toFixed(2)
            : "0.00",
        },
      }, null, 2)
    }]
